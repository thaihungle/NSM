...
parser.add_argument('--train_features_h5', default='data/train_features_pool_clips.h5')
parser.add_argument('--val_question_h5', default='data/val_questions.h5')
parser.add_argument('--val_features_h5', default='data/val_features_pool_clips.h5')
parser.add_argument('--test_question_h5', default='data/test_questions.h5')
parser.add_argument('--test_features_h5', default='data/test_features_pool_clips.h5')
...
parser.add_argument('--model_type', default='MANN',
        choices=['PG', 'EE', 'PG+EE', 'LSTM', 'CNN+LSTM', 'CNN+LSTM+SA', 'MANN'])
...
parser.add_argument('--module_dim', default=64, type=int)
...
# Classifier options
parser.add_argument('--classifier_proj_dim', default=128, type=int)
parser.add_argument('--classifier_downsample', default='maxpool2',
        choices=['maxpool2', 'maxpool4', 'none'])
parser.add_argument('--classifier_fc_dims', default='256')
parser.add_argument('--classifier_batchnorm', default=0, type=int)
parser.add_argument('--classifier_dropout', default=0, type=float)

# Optimization options
parser.add_argument('--batch_size', default=64, type=int)
parser.add_argument('--num_iterations', default=29240, type=int)
...
parser.add_argument('--checkpoint_every', default=1462, type=int) # 1462
parser.add_argument('--summary_dir', default='summary')
parser.add_argument('--exp_name', default='dnc-debug')
torch.manual_seed(1)
...
def main(args):
  ...
  with ClevrDataLoader(**train_loader_kwargs) as train_loader, \
       ClevrDataLoader(**val_loader_kwargs) as val_loader, ClevrDataLoader(**test_loader_kwargs) as test_loader:
    train_loop(args, train_loader, val_loader, test_loader)
...
def train_loop(args, train_loader, val_loader, test_loader):
  ...
  if args.model_type == 'EE' or args.model_type == 'PG+EE':
    ...
    pytorch_total_params = sum(p.numel() for p in execution_engine.parameters() if p.requires_grad)
  ...
  if args.model_type in ['LSTM', 'CNN+LSTM', 'CNN+LSTM+SA']:
    ...
    dnc = DNC(video_dim=2048,
              wordvec_dim=256,
              input_size=1024,
              final_output_size=1024,
              hidden_size=256,
              vocab=vocab)

  if args.model_type == "MANN":
      dnc = DNC(video_dim=2048,
            wordvec_dim= 256,
            input_size=1024,
            final_output_size=1024,
            ...
            vocab= vocab)
      print("DNC created!")
      pytorch_total_params = sum(p.numel() for p in dnc.parameters() if p.requires_grad)
      print("\ntotal_params:",pytorch_total_params)

      dnc.cuda()
      params = dnc.parameters()
      dnc_optimizer = torch.optim.Adam(params, lr=args.learning_rate)
  ...
  curve_dir = os.path.join(args.summary_dir, args.exp_name)

  if not os.path.exists(curve_dir):
    os.makedirs(curve_dir)

  writer = SummaryWriter(curve_dir)
    ...
    for batch in train_loader:
      ...
      if args.model_type == 'PG':
        ...
        if args.train_program_generator == 1:
          ...
          pg_optimizer.step()
      elif args.model_type == "MANN":
          dnc_optimizer.zero_grad()
          dnc.zero_grad()

          scores = dnc(feats_var, questions_var)
          loss = loss_fn(scores, answers_var)
          loss.backward()
          dnc_optimizer.step()
      ...
      if t % args.record_loss_every == 0:
        ...
        writer.add_scalar("Train/loss", loss.data[0], t)
      ...
      if t % args.checkpoint_every == 0:
        ...
        train_acc = check_accuracy(args, program_generator, execution_engine,
                                   baseline_model, dnc, train_loader)
        print('train accuracy is', train_acc)
        writer.add_scalar('Train/acc', train_acc, t)

        print('Checking validation accuracy ...')
        val_acc = check_accuracy(args, program_generator, execution_engine,
                                 baseline_model, dnc, val_loader)
        print('val accuracy is ', val_acc)
        writer.add_scalar('Val/acc', val_acc, t)
        stats['train_accs'].append(train_acc)
        stats['val_accs'].append(val_acc)
        stats['val_accs_ts'].append(t)

        test_acc = check_accuracy(args, program_generator, execution_engine,
                                  baseline_model, dnc, test_loader)
...
def check_accuracy(args, program_generator, execution_engine, baseline_model, mann_model,loader):
  set_mode('eval', [program_generator, execution_engine, baseline_model, mann_model])
  ...
  for batch in loader:
    ...
    if args.model_type == 'PG':
      ...
      scores = baseline_model(questions_var, feats_var)
        ...
        scores = mann_model(feats_var, questions_var)
...
if __name__ == '__main__':
  ...
  print("Training take {} (s)".format(end - start))
